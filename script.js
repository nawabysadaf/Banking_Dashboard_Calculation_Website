...
const account1 = {
  ...
  movementsDates: [
    '2025-08-18T21:31:17.178Z',
    '2025-08-23T07:42:02.383Z',
...
const account2 = {
  ...
  movementsDates: [
    '2025-09-01T13:15:33.035Z',
    '2025-08-30T09:48:16.867Z',
...
const displayMovements = function (acc, sort = false) {
  ...
  movementSort.forEach(function (mov, i) {
    ...
    const displayDate = calcDate(date, currentAccount.locale);
    const formatedNumber = calcCurrency(currentAccount, mov);
    const html = `
...
const calcDate = function (date, locale) {
  ...
  const calcDay = calcDayPassed(new Date(), date);
  ...
  if (calcDay > 7) return dateFormat;
};

// calculate currency
const calcCurrency = function (acc, value) {
...
const displayBalance = function (acc) {
  ...
  labelBalance.textContent = calcCurrency(currentAccount, acc.balance);
};

// calculate balance summary function
const displayBalanceSummary = function (acc) {
  const income = acc.movements
    .filter(mov => mov > 0)
    .reduce((acc, cur) => acc + cur, 0);
  labelSumIn.textContent = calcCurrency(currentAccount, income);

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, cur) => acc + cur, 0);
  labelSumOut.textContent = calcCurrency(currentAccount, out);
  ...
  const rate = acc.movements
    ...
    .filter(mov => mov >= 1)
    .reduce((acc, cur) => acc + cur, 0);
  labelSumInterest.textContent = calcCurrency(currentAccount, rate);
};

// calculate intrestRate balance function
const displayIntrestRate = function (movements) {};

// creating username for each account function
const createUserName = function (account) {
  account.forEach(function (name) {
...
btnTransfer.addEventListener('click', function (e) {
  e.preventDefault();
  // get amount and recierver and find recierver
